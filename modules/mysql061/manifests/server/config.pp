...
#            mysql061::server because it needs some variables
#            out of the mysql061::config class which will be
#            included!
#
# == Parameters:
#
...
#   Easy one:
#
#   mysql061::server::config { 'basic_config':
...
#   port = 3300
#
define mysql061::server::config (
  $settings,
  $notify_service = true
) {
  include mysql061::config
  ...
  file { "/etc/mysql/conf.d/${name}.cnf":
    ...
    content => $content,
    owner   => 'root',
    group   => $mysql061::config::root_group,
    mode    => '0644',
